...
namespace SPTAG
{
    ...
    namespace BKT
    {
        ...
        private:
            ...
            std::function<float(const T* pX_f, const T* pY_f, DimensionType dimension_f,SizeType filtersNum, const T* pX, const T* pY, DimensionType dimension)> m_fComputeDistance;
            int m_iBaseSquare;
...
#undef DefineBKTParameter
                ...
                m_fComputeDistance = std::function<float(const T* pX_f, const T* pY_f, DimensionType dimension_f,SizeType filtersNum, const T* pX, const T* pY, DimensionType dimension)>(COMMON::DistanceCalcSelector<T>(m_iDistCalcMethod));
                m_iBaseSquare = (m_iDistCalcMethod == DistCalcMethod::Cosine) ? COMMON::Utils::GetBase<T>() * COMMON::Utils::GetBase<T>() : 1;
            }

            ~Index() {}

            const void* GetFilter(const SizeType idx) const {
                auto filter = m_filterdataSet_->GetFilterdata(idx);
                return filter.Data();
            }

            const SizeType GetFilterNum () const {
                return m_filterdataSet_->GetFilterNum();
            }

            const DimensionType GetFilterDim () const {
                return m_filterdataSet_->GetFilterDim();
            }

            const void SetFilterForDataset(const std::shared_ptr<SPTAG::FilterdataSet> filterData) {
                printf("SetFilterForDataset from BKT\n");
                m_pSamples.setFilterdataSet(filterData);
            }
            ...
            inline float AccurateDistance(const void* pX, const void* pY) const { 
                if (m_iDistCalcMethod == DistCalcMethod::L2) return m_fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1, (SPTAG::SizeType)1 ,(const T*)pX, (const T*)pY, m_pSamples.C());

                float xy = m_iBaseSquare - m_fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1, (SPTAG::SizeType)1 ,(const T*)pX, (const T*)pY, m_pSamples.C()); //TODO change parameter
                float xx = m_iBaseSquare - m_fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1, (SPTAG::SizeType)1 ,(const T*)pX, (const T*)pX, m_pSamples.C());
                float yy = m_iBaseSquare - m_fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1, (SPTAG::SizeType)1 ,(const T*)pY, (const T*)pY, m_pSamples.C()); 
                return 1.0f - xy / (sqrt(xx) * sqrt(yy));
            }
            inline float ComputeDistance(const void* pX_f, const void* pY_f,const void* pX, const void* pY) const { return m_fComputeDistance((T*)pX_f,(T*) pY_f, m_filterdataSet_->GetFilterDim(),  m_filterdataSet_->GetFilterNum(),(const T*)pX, (const T*)pY, m_pSamples.C()); }
            ...
            ErrorCode BuildIndexWFilters(const void* p_data, SizeType p_vectorNum, DimensionType p_dimension,  const void* p_filters, SizeType p_filtersNum, DimensionType p_filtersDimension, bool p_normalized = false, bool p_shareOwnership = false){
                return ErrorCode::Success;
            };
            ErrorCode SearchIndexWFilters(QueryResult &p_query, bool p_searchDeleted = false) const {
            printf("entered the BKT search function\n");
                return ErrorCode::Success;
            };

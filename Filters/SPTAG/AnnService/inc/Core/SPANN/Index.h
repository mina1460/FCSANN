...
namespace SPTAG
{
    ...
    namespace SPANN
    {
        ...
        private:
            ...
            std::function<float(const T* , const T* , DimensionType ,SizeType , const T* , const T* , DimensionType )> m_fComputeDistance;
            int m_iBaseSquare;

            std::shared_ptr<FilterdataSet> m_filterdataSet;

        public:
            Index()
            {
                m_fComputeDistance = std::function<float(const T* , const T* , DimensionType ,SizeType , const T* , const T* , DimensionType)>(COMMON::DistanceCalcSelector<T>(m_options.m_distCalcMethod));
            ...
            inline float AccurateDistance(const void* pX, const void* pY) const { 
                if (m_options.m_distCalcMethod == DistCalcMethod::L2) return m_fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1, (SPTAG::SizeType)1 ,(const T*)pX, (const T*)pY, m_options.m_dim);

                float xy = m_iBaseSquare - m_fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1, (SPTAG::SizeType)1 ,(const T*)pX, (const T*)pY, m_options.m_dim);
                float xx = m_iBaseSquare - m_fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1, (SPTAG::SizeType)1 ,(const T*)pX, (const T*)pX, m_options.m_dim);
                float yy = m_iBaseSquare - m_fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1, (SPTAG::SizeType)1 ,(const T*)pY, (const T*)pY, m_options.m_dim);
                return 1.0f - xy / (sqrt(xx) * sqrt(yy));
            }
            inline float ComputeDistance(const void* pX_f, const void* pY_f, const void* pX, const void* pY) const { return m_fComputeDistance((T*)pX_f,(T*) pY_f, m_filterdataSet->GetFilterDim(),  m_filterdataSet->GetFilterNum(),(const T*)pX, (const T*)pY, m_options.m_dim); }
            ...
            // ErrorCode BuildIndexWFilters(bool p_normalized = false);
        ...
        private:
            ...
            ErrorCode BuildIndexInternalWFilters(std::shared_ptr<Helper::VectorSetReader>& p_reader);
        };
    } // namespace SPANN
} // namespace SPTAG

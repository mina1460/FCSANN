...
#define ProcessPosting()                                                                                                                                                                    \
    ...
    {                                                                                                                                                                                       \
        ...
        SPTAG::COMMON::filterWeight = 6.0;                                                                                                                                                  \
        auto distance2leaf = p_index->ComputeDistance(queryResults.GetFilterData(), p_index->GetFilter(vectorID), queryResults.GetQuantizedTarget(), p_postingListFullData + offsetVector); \
        SPTAG::COMMON::filterWeight = 1.0;                                                                                                                                                  \
                                     ...
                                     std::set<int> *truth, std::map<int, std::set<int>> *found)
            {
                ...
                for (uint32_t pi = 0; pi < postingListCount; ++pi)
                {
                    ...
                    request.m_callback = [&p_exWorkSpace, &queryResults, &p_index, &request, this](bool success)
                    {
                        ...
                        // ComputeDistance(NULL, NULL, queryResults.GetQuantizedTarget(), p_postingListFullData + offsetVector);
                        ProcessPosting();
                    };
#else // async read
...
#pragma omp parallel for schedule(dynamic)
                        ...
                        {
                            COMMON::Utils::atomic_float_add(&acc, COMMON::TruthSet::CalculateRecall(p_headIndex.get(), fullVectors->GetVector(samples[j]), candidateNum, samples[j]));
                        }
                        acc = acc / sampleNum;

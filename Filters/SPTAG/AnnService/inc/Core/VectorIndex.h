...
public:
    ...
    virtual float ComputeDistance(const void* pX_f, const void* pY_f, const void* pX, const void* pY) const = 0;
    virtual const void* GetSample(const SizeType idx) const = 0;
    virtual const void* GetFilter(const SizeType idx) const ;
    virtual const SizeType GetFilterNum () const;
    virtual const void SetFilterForDataset(const std::shared_ptr<SPTAG::FilterdataSet> filterData) ;
    virtual const DimensionType GetFilterDim () const;
    virtual const void SetFilter(const std::shared_ptr<SPTAG::FilterdataSet> filterData) ;
    ...
    virtual ErrorCode SearchIndexWFilters(QueryResult &p_query, bool p_searchDeleted = false) const =0;
    ...
    virtual ErrorCode BuildIndexWFilters(const void* p_data, SizeType p_vectorNum, DimensionType p_dimension,  const void* p_filters, SizeType p_filtersNum, DimensionType p_filtersDimension, bool p_normalized = false, bool p_shareOwnership = false){return ErrorCode::Undefined;};
...
protected:
    ...
    std::shared_ptr<SPTAG::FilterdataSet> m_filterdataSet_;

...
#pragma omp parallel for schedule(dynamic)
                for (SizeType i = 0; i < samples; i++)
                {
                    ...
                    for (SizeType y = 0; y < m_iGraphSize; y++)
                    {
                        ...
                        float dist = index->ComputeDistance(index->GetFilter(x), index->GetFilter(y), index->GetSample(x), index->GetSample(y));
                    ...
                    for (SizeType j = 0; j < (SizeType)TptreeLeafNodes[i].size(); j++)
                    {
                        ...
                        for (SizeType x = start_index; x < end_index; x++)
                        {
                            for (SizeType y = x + 1; y <= end_index; y++)
                            {
                                ...
                                float dist = index->ComputeDistance(index->GetFilter(p1), index->GetFilter(p2), index->GetSample(p1), index->GetSample(p2));
                                if (idmap != nullptr) {
...
#pragma omp parallel for schedule(dynamic)
            ...
            void RefineNode(VectorIndex* index, const SizeType node, bool updateNeighbors, bool searchDeleted, int CEF)
            {
                ...
                query.InitFilters(index->GetFilter(node), index->GetFilterNum(), index->GetFilterDim());

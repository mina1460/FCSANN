...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        {
            printf("***PQQuantizer:::QuantizeVector\n");
            if (GetEnableADC())
            {
                ...
                for (int i = 0; i < m_NumSubvectors; i++)
                {
                    ...
                    {
                        (*ADCtable) = distCalc((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,subvec, subcodebooks, m_DimPerSubvector);
                        ADCtable++;
            ...
            else 
            {
                ...
                for (int i = 0; i < m_NumSubvectors; i++) {
                    ...
                    for (int j = 0; j < m_KsPerSubvector; j++) {
                        float dist = distCalc((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,subvec, subcodebooks, m_DimPerSubvector);
                        if (dist < minDist) {
                            bestIndex = j;
                            minDist = dist;
                        }
        ...
        {
            printf("***PQQuantizer:::InitializeDistanceTables\n");
            ...
            for (int i = 0; i < m_NumSubvectors; i++) {
                ...
                for (int j = 0; j < m_KsPerSubvector; j++) {
                    for (int k = 0; k < m_KsPerSubvector; k++) {
                        temp_m_L2DistanceTables[m_DistIndexCalc(i, j, k)] = L2Dist((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,&m_codebooks[baseIdx + j * m_DimPerSubvector], &m_codebooks[baseIdx + k * m_DimPerSubvector], m_DimPerSubvector);
                    }
                }
            }

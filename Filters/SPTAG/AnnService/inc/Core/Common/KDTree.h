...
#pragma omp parallel for num_threads(numOfThreads)
            ...
            void InitSearchTrees(const Dataset<T>& p_data, std::function<float(const T* pX_f, const T* pY_f, DimensionType dimension_f,SizeType filtersNum, const T* pX, const T* pY, DimensionType dimension)> fComputeDistance, COMMON::QueryResultSet<T> &p_query, COMMON::WorkSpace &p_space) const
            {
                for (int i = 0; i < m_iTreeNumber; i++) {
            ...
            template <typename T, typename Q>
            void SearchTrees(const Dataset<T>& p_data, std::function<float(const T* pX_f, const T* pY_f, DimensionType dimension_f,SizeType filtersNum, const T* pX, const T* pY, DimensionType dimension)> fComputeDistance, COMMON::QueryResultSet<T> &p_query, COMMON::WorkSpace &p_space, const int p_limits) const
            {
        ...
        private:
            ...
            template <typename T, typename Q>
            void KDTSearch(const Dataset<T>& p_data, std::function<float(const T* , const T* , DimensionType ,SizeType , const T* , const T* , DimensionType )> fComputeDistance, COMMON::QueryResultSet<T> &p_query,
                ...
                if (node < 0)
                {
                    ...
                    p_space.m_NGQueue.insert(NodeDistPair(index, fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,p_query.GetQuantizedTarget(), data, p_data.C())));
                    return;
                }

...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        public:
                    ...
                    for (SizeType id : truth[i])
                    {
                        for (int j = 0; j < K; j++)
                        {
                            ...
                            if (results[i].GetResult(j)->VID == id)
                            {
                                ...
                                float dist = COMMON::DistanceUtils::ComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,(const T*)querySet->GetVector(i), (const T*)vectorSet->GetVector(results[i].GetResult(j)->VID), vectorSet->Dimension(), index->GetDistCalcMethod());
                                float truthDist = COMMON::DistanceUtils::ComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,(const T*)querySet->GetVector(i), (const T*)vectorSet->GetVector(id), vectorSet->Dimension(), index->GetDistCalcMethod());
                    ...
                    if (debug) {
                        ...
                        for (SizeType id : truth[i]) {
                            ...
                            if (vectorSet != nullptr) {
                                truthDist = COMMON::DistanceUtils::ComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,(const T*)querySet->GetVector(i), (const T*)vectorSet->GetVector(id), querySet->Dimension(), index->GetDistCalcMethod());
                            }
            ...
            template <typename T>
            static float CalculateRecall(VectorIndex* index, T* query, int K, int VID) {
                ...
                sampleANN.SetFilter(index->GetFilter(VID));

                index->SearchIndex(sampleANN);
                for (SizeType y = 0; y < index->GetNumSamples(); y++)
                {
                    // printf("Y:%d\n", y);
                    float dist = index->ComputeDistance(index->GetFilter(VID), index->GetFilter(y), sampleTruth.GetQuantizedTarget(), index->GetSample(y));

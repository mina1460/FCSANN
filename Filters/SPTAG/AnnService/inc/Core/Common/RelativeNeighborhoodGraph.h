        ...
        public:
            ...
            void RebuildNeighbors(VectorIndex* index, const SizeType node, SizeType* nodes, const BasicResult* queryResults, const int numResults) {
                ...
                for (int j = 0; j < numResults && count < m_iNeighborhoodSize; j++) {
                    ...
                    for (DimensionType k = 0; k < count; k++) {
                        if (m_fRNGFactor * index->ComputeDistance(index->GetFilter(nodes[k]), index->GetFilter(item.VID), index->GetSample(nodes[k]), index->GetSample(item.VID)) < item.Dist) {
                            good = false;
                            break;
                        }
                    }
            ...
            {                
                ...
                for (DimensionType k = 0; k < checkNeighborhoodSize; k++)
                {
                    ...
                    tmpDist = index->ComputeDistance(NULL, NULL, tmpVec, nodeVec);
                    if (tmpDist > insertDist || (insertDist == tmpDist && insertNode < tmpNode))
                    {
                        nodes[k] = insertNode;
                        while (++k < checkNeighborhoodSize && index->ComputeDistance(NULL, NULL, tmpVec, nodeVec) <= index->ComputeDistance(NULL, NULL, tmpVec, insertVec)) {
                    ...
                    else if (index->ComputeDistance(NULL, NULL, tmpVec, insertVec) < insertDist) {
                        break;
                    }
                }
            }

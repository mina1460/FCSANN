...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        struct KmeansArgs {
            ...
            std::function<float(const T* pX_f, const T* pY_f, DimensionType dimension_f,SizeType filtersNum, const T* pX, const T* pY, DimensionType dimension)> fComputeDistance;
            const std::shared_ptr<IQuantizer>& m_pQuantizer;

            float* newCentersFilters;       //NOTE: added
            T* centersFilters;
            T* newTCentersFilters;
            DimensionType _DF;    // dimension of each filter
            SizeType _CF;  // number of filters per data node


            KmeansArgs(int k, DimensionType dim, SizeType datasize, int threadnum, DistCalcMethod distMethod, const std::shared_ptr<IQuantizer>& quantizer = nullptr) 
                : _K(k), _DK(k), _D(dim), _RD(dim), _T(threadnum), _M(distMethod), m_pQuantizer(quantizer){
                
                if (m_pQuantizer) {
                    _RD = m_pQuantizer->ReconstructDim();
                    // fComputeDistance = m_pQuantizer->DistanceCalcSelector<T>(distMethod); //TODO uncomment
                }
                ...
                newCentersFilters = new float[_T * _K * _DF * _CF];       //NOTE: added
            }

            KmeansArgs(int k, DimensionType dim, SizeType datasize, DimensionType dimFilter, SizeType numFilter, int threadnum, DistCalcMethod distMethod, const std::shared_ptr<IQuantizer>& quantizer = nullptr) 
                : _K(k), _DK(k), _D(dim), _RD(dim), _DF(dimFilter), _CF(numFilter), _T(threadnum), _M(distMethod), m_pQuantizer(quantizer){
                ...
                newCentersFilters = new float[_T * _K * _DF * _CF];       //NOTE: added
                centersFilters = (T*)ALIGN_ALLOC(sizeof(T) * _K * _DF * _CF);
                newTCentersFilters = (T*)ALIGN_ALLOC(sizeof(T) * _K * _DF * _CF);
            }
            ...
            ~KmeansArgs() {
                ...
                delete[] newCentersFilters;       //NOTE: added
            }
            ...
            inline void ClearCenters() {
                ...
                memset(newCentersFilters, 0, sizeof(float) * _T * _K * _DF * _CF);       //NOTE: added
            }
        ...
        {
            std::shared_ptr<SPTAG::FilterdataSet> m_filterdataSet = data.GetFilterdataSet();

            int maxcluster = -1;
            SizeType maxCount = 0;
            for (int k = 0; k < args._DK; k++) {
                if (args.counts[k] > maxCount && args.newCounts[k] > 0 && DistanceUtils::ComputeDistance((T*)data.GetFilterdataSet(args.clusterIdx[k]).Data(),args.centersFilters + k * args._DF * args._CF, args._DF, args._CF,(T*)data[args.clusterIdx[k]], args.centers + k * args._D, args._D, DistCalcMethod::L2) > 1e-6) //TODO change parameters
                {
                    maxcluster = k;
                    maxCount = args.counts[k];
                }
            ...
            for (int k = 0; k < args._DK; k++) {
                ...
                T* TCentersFilters = args.newTCentersFilters + k * args._DF * args._CF; // NOTE: added
                if (args.counts[k] == 0) {
                    if (maxcluster != -1) {
                        ...
                        std::memcpy(TCentersFilters, data.GetFilterdataSet(nextid).Data(), sizeof(T)*args._DF * args._CF); // NOTE: added
                    }
                    else {
                        std::memcpy(TCenter, args.centers + k * args._D, sizeof(T)*args._D);
                        std::memcpy(TCentersFilters, args.centersFilters + k * args._DF * args._CF, sizeof(T)*args._DF * args._CF); // NOTE: added
                    }
                }
                else {
                    float* currCenters = args.newCenters + k * args._RD;
                    float* currCentersFilters = args.newCentersFilters + k * args._DF * args._CF; // NOTE: added
                    for (DimensionType j = 0; j < args._RD; j++) {
                        currCenters[j] /= args.counts[k];
                    }
                    for (DimensionType j = 0; j < args._DF * args._CF; j++){ // NOTE: added
                        currCentersFilters[j] /= args.counts[k]; // NOTE: added
                    }
                    /*
                    ...
                    */
                    if (args.m_pQuantizer) {
                        ...
                        for (DimensionType j = 0; j < args._DF * args._CF; j++) 
                            TCentersFilters[j] = (T)(currCentersFilters[j]);  // NOTE: added
                    }
                }
                // diff += args.fComputeDistance(args.centersFilters + k*args._DF*args._CF, NULL, args._DF, args._CF, args.centers + k*args._D, TCenter, args._D); //TODO change parameters
                diff += args.fComputeDistance(args.centersFilters + k*args._DF*args._CF, TCentersFilters, args._DF, args._CF, args.centers + k*args._D, TCenter, args._D); //NOTE added
            }
            return diff;
        }
...
#pragma omp parallel for num_threads(args._T) shared(data, indices) reduction(+:currDist)
            for (int tid = 0; tid < args._T; tid++)
            {
                ...
                float *inewCentersFilters = args.newCentersFilters + tid * args._K * args._DF * args._CF; // NOTE: added
                ...
                R* reconstructVectorFilters = nullptr; // NOTE: added
                ...
                for (SizeType i = istart; i < iend; i++) {
                    ...
                    for (int k = 0; k < args._DK; k++) {
                        float dist = args.fComputeDistance((T*)data.GetFilterdataSet(indices[i]).Data(),args.centersFilters + k*args._DF*args._CF, args._DF, args._CF, data[indices[i]], args.centers + k*args._D, args._D) + lambda*args.counts[k]; //TODO fix this
                    ...
                    if (updateCenters) {
                        ...
                        reconstructVectorFilters = (R*)data.GetFilterdataSet(indices[i]).Data(); // NOTE: added
                        float* center = inewCenters + clusterid*args._RD;
                        float* centerFilters = inewCentersFilters + clusterid*args._DF*args._CF; // NOTE: added
                        for (DimensionType j = 0; j < args._RD; j++) center[j] += reconstructVector[j];
                        for (DimensionType j = 0; j < args._DF * args._CF; j++) centerFilters[j] += reconstructVectorFilters[j]; // NOTE: added
            ...
            if (updateCenters) {
                for (int i = 1; i < args._T; i++) {
                    ...
                    float* currCenterFilter = args.newCentersFilters + i*args._K*args._DF*args._CF; //NOTE added
                    for (size_t j = 0; j < ((size_t)args._DK) * args._RD; j++) args.newCenters[j] += currCenter[j];
                    for (size_t j = 0; j < ((size_t)args._DK) * args._DF*args._CF; j++) args.newCentersFilters[j] += currCenterFilter[j]; //NOTE added
        ...
        inline float InitCenters(const Dataset<T>& data, 
            ...
            for (int numKmeans = 0; numKmeans < tryIters; numKmeans++) {
                for (int k = 0; k < args._DK; k++) {
                    ...
                    std::memcpy(args.centersFilters + k*args._DF*args._CF, data.GetFilterdataSet(indices[randid]).Data(), sizeof(T)*args._DF*args._CF);
                }
                args.ClearCounts();
                ...
                if (currDist < minClusterDist) {
                    ...
                    memcpy(args.newTCentersFilters, args.centersFilters, sizeof(T)*args._K*args._DF*args._CF);
        ...
        float TryClustering(const Dataset<T>& data,
            std::vector<SizeType>& indices, const SizeType first, const SizeType last,
            KmeansArgs<T>& args,  std::shared_ptr<SPTAG::FilterdataSet> m_filterdataSet,int samples = 1000, float lambdaFactor = 100.0f, bool debug = false, IAbortOperation* abort = nullptr) {
            ...
            for (int iter = 0; iter < 100; iter++) {
                ...
                memcpy(args.centersFilters, args.newTCentersFilters, sizeof(T)*args._K*args._DF*args._CF);
            ...
            for (int k = 0; k < args._DK; k++) {
                if (args.clusterIdx[k] != -1){ 
                    ...
                    std::memcpy(args.centersFilters + k * args._DF * args._CF, data.GetFilterdataSet(args.clusterIdx[k]).Data(), sizeof(T) * args._DF * args._CF);
                }
            }
...
CountStd = TryClustering<T, Type>(data, indices, first, last, args, m_filterdataSet, samples, lambdaFactor, true); \
break;
...
#undef DefineVectorValueType
                ...
                {
                    CountStd = TryClustering<T, T>(data, indices, first, last, args,m_filterdataSet, samples, lambdaFactor, true);
                }
...
#define DefineVectorValueType(Name, Type) \
case VectorValueType::Name: \
TryClustering<T, Type>(data, indices, first, last, args, m_filterdataSet, samples, lambdaFactor, debug, abort); \
break;
...
#undef DefineVectorValueType
            ...
            {
                TryClustering<T, T>(data, indices, first, last, args, data.GetFilterdataSet(), samples, lambdaFactor, debug, abort);
            }
        ...
        public:
            ...
            void Rebuild(const Dataset<T>& data, DistCalcMethod distMethod, IAbortOperation* abort)
            {
                ...
                newTrees.BuildTrees<T>(data, distMethod, 1, data.GetFilterdataSet(), nullptr, nullptr, false, abort);
            ...
            template <typename T>
            void BuildTrees(const Dataset<T>& data, DistCalcMethod distMethod, int numOfThreads, std::shared_ptr<SPTAG::FilterdataSet> m_filterdataSet,
                ...
                bool dynamicK = false, IAbortOperation* abort = nullptr)
            {
                ...
                KmeansArgs<T> args(m_iBKTKmeansK, data.C(), (SizeType)localindices.size(), m_filterdataSet->GetFilterDim(), m_filterdataSet->GetFilterNum(), numOfThreads, distMethod, m_pQuantizer);
                ...
                for (char i = 0; i < m_iTreeNumber; i++)
                {
                    ...
                    while (!ss.empty()) {
                        ...
                        if (item.last - item.first <= m_iBKTLeafSize) {
                            ...
                            int numClusters = KmeansClustering(data, localindices, item.first, item.last, args,m_filterdataSet, m_iSamples, m_fBalanceFactor, item.debug, abort);
                            if (numClusters <= 1) {
            ...
            template <typename T>
            void InitSearchTrees(const Dataset<T>& data, std::function<float(const T* pX_f, const T* pY_f, DimensionType dimension_f,SizeType filtersNum, const T* pX, const T* pY, DimensionType dimension)> fComputeDistance, COMMON::QueryResultSet<T> &p_query, COMMON::WorkSpace &p_space, std::shared_ptr<FilterdataSet> m_filterdataSet) const
            {   
                for (char i = 0; i < m_iTreeNumber; i++) {
                    const BKTNode& node = m_pTreeRoots[m_pTreeStart[i]];
                    if (node.childStart < 0) {
                        p_space.m_SPTQueue.insert(NodeDistPair(m_pTreeStart[i], fComputeDistance((T*)p_query.GetFilterData(),(T*)data.GetFilterdataSet(node.centerid).Data(), m_filterdataSet->GetFilterDim(), m_filterdataSet->GetFilterNum(), p_query.GetQuantizedTarget(), data[node.centerid], data.C())));     //TODO: check if node.centerid is the correct vector id
                    } else if (m_bfs) {
                        float FactorQ = 1.1f;
                        int MaxBFSNodes = 100;
                        ...
                        for (SizeType begin = node.childStart; begin < node.childEnd; begin++) {
                            ...
                            float dist = fComputeDistance((T*)p_query.GetFilterData(),(T*)data.GetFilterdataSet(index).Data(), m_filterdataSet->GetFilterDim(), m_filterdataSet->GetFilterNum() ,p_query.GetQuantizedTarget(), data[index], data.C()); //TODO
                            ...
                            while (!p_curr->empty()) {
                                ...
                                if (tnode.childStart < 0) {
                                    ...
                                    for (SizeType begin = tnode.childStart; begin < tnode.childEnd; begin++) {
                                        SizeType index = m_pTreeRoots[begin].centerid;
                                        float dist = fComputeDistance((T*)p_query.GetFilterData(),(T*)data.GetFilterdataSet(index).Data(), m_filterdataSet->GetFilterDim(), m_filterdataSet->GetFilterNum(),p_query.GetQuantizedTarget(), data[index], data.C());
                        ...
                        for (SizeType begin = node.childStart; begin < node.childEnd; begin++) {
                            SizeType index = m_pTreeRoots[begin].centerid;
                            p_space.m_SPTQueue.insert(NodeDistPair(begin, fComputeDistance((T*)p_query.GetFilterData(),(T*)data.GetFilterdataSet(index).Data(), data.GetFilterdataSet()->GetFilterDim(), data.GetFilterdataSet()->GetFilterNum(), p_query.GetQuantizedTarget(), data[index], data.C())));
                        }
                    }
                }
            }
            ...
            template <typename T>
            void SearchTrees(const Dataset<T>& data, std::function<float(const T* pX_f, const T* pY_f, DimensionType dimension_f,SizeType filtersNum, const T* pX, const T* pY, DimensionType dimension)> fComputeDistance, COMMON::QueryResultSet<T> &p_query,
                COMMON::WorkSpace &p_space, const int p_limits, std::shared_ptr<FilterdataSet> m_filterdataSet) const
            {

...
public:
    ...
    inline void SetFilterData(int p_index, ByteArray p_filterData)
    {
        if (p_index < m_resultNum)
        {
            m_results[p_index].Filters = std::move(p_filterData);
        }
    }

    inline void SetFilter (const void* p_filter){
        m_filters = p_filter;
    }

    void InitFilters(const void * p_filters, SizeType p_filterNum, DimensionType p_filtersDimension){
        m_filters = p_filters;
        m_filterNum = p_filterNum;
        m_filtersDimension = p_filtersDimension;
    }


    inline ByteArray GetFilterData(int p_index) const
    {
        if (p_index < m_resultNum)
        {
            return m_results[p_index].Filters;
        }

        return ByteArray::c_empty;
    }

    inline const void* GetFilterData()
    {
        return m_filters;
    }

    inline SizeType GetFilterNum()
    {
        return m_filterNum;
    }

    inline DimensionType GetFilterDim()
    {
        return m_filtersDimension;
    }
...
protected:
    const void* m_target;       // query data

    void* m_quantizedTarget;

    SizeType m_quantizedSize;

    int m_resultNum;        // number of neighbors we want (k)

    bool m_withMeta;

    Array<BasicResult> m_results;       // the nearest k neighbors

    const void* m_filters;      // query filter data

    SizeType m_filterNum;

    DimensionType m_filtersDimension;
};
} // namespace SPTAG

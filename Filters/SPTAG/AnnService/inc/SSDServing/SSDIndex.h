                ...
                if (p_opts.m_rerank > 0 && vectorSet != nullptr) {
                    ...
                    for (int i = 0; i < results.size(); i++)
                    {
                        for (int j = 0; j < K; j++)
                        {
                            ...
                            results[i].GetResult(j)->Dist = COMMON::DistanceUtils::ComputeDistance((ValueType*)NULL,(ValueType*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,(const ValueType*)querySet->GetVector(i),
...
#pragma omp parallel for schedule(dynamic)
                    for (int i = 0; i < sampleSize; i++)
                    {
                        ...
                        {
                            float dist = headIndex->ComputeDistance(NULL, NULL, queryBFHeads.GetQuantizedTarget(), headIndex->GetSample(y));
                        ...
                        for (std::map<int, std::set<int>>::iterator it = tmpFound.begin(); it != tmpFound.end(); it++) {
                            float q2truthposting = headIndex->ComputeDistance(NULL, NULL, querySet->GetVector(samples[i]), headIndex->GetSample(it->first));
                            for (auto vid : it->second) {
                                ...
                                if (q2truthposting < queryANNHeadsLongestDist) shouldSelect[i] += 1;
                                    ...
                                    float v2vhead = headIndex->ComputeDistance(NULL, NULL, vectorSet->GetVector(vid), headIndex->GetSample(it->first));
                                    for (SizeType z = 0; z < internalResultNum; z++) {
                                        if (queryANNHeads.GetResult(z)->VID < 0) break;
                                        float v2qhead = headIndex->ComputeDistance(NULL, NULL, vectorSet->GetVector(vid), headIndex->GetSample(queryANNHeads.GetResult(z)->VID));
                                        if (v2qhead < v2vhead) {
                                    ...
                                    for (SizeType z = 0; z < annTruthHead.GetResultNum() && replicas.size() < p_opts.m_replicaCount; z++) {
                                        ...
                                        bool good = true;
                                        for (auto r : replicas) {
                                            if (p_opts.m_rngFactor * headIndex->ComputeDistance(NULL, NULL, headIndex->GetSample(r), headIndex->GetSample(item->VID)) < item->Dist) {
                                                good = false;
                                                break;
                                            }
                                        }

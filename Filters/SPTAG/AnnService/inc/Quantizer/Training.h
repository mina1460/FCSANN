...
#pragma omp parallel for
    for (int codebookIdx = 0; codebookIdx < options->m_quantizedDim; codebookIdx++) {
        ...
        auto nclusters = COMMON::KmeansClustering<T>(dset, localindices, 0, dset.R(), kargs, std::make_shared<SPTAG::MemFilterdataSet>(), options->m_trainingSamples, options->m_KmeansLambda, options->m_debug, nullptr);

...
namespace SPTAG
{
    namespace Helper
    {
        ...
        public:
            ...
            virtual std::shared_ptr<FilterdataSet> GetFilterdataSet() const { return nullptr; }

            virtual ErrorCode LoadFilterVectorFile(const std::string& p_filePath) { return ErrorCode::Success; }

            virtual std::shared_ptr<VectorSet> GetFilterVectorSet(SizeType start = 0, SizeType end = -1) const  //todo
            {
                if (end < 0 || end > m_vectors->FiltersCount()) end = m_vectors->FiltersCount();
                return std::shared_ptr<VectorSet>(new BasicVectorSet(ByteArray((std::uint8_t*)(m_vectors->GetVector(start)), (end - start) * m_vectors->PerVectorDataSize(), false),
                    m_vectors->GetValueType(),
                    m_vectors->Dimension(),
                    end - start));

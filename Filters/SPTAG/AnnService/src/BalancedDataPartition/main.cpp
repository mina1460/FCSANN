...
inline float MultipleClustersAssign(const COMMON::Dataset<T>& data,
    ...
    auto func = [&](int tid)
    {
        ...
        printf("***main.cpp:::MultipleClustersAssign\n");
        for (SizeType i = istart; i < iend; i++) {
            for (int k = 0; k < args._K; k++) {
                ...
                float dist = args.fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,data[indices[i]], args.centers + k * args._D, args._D) + penalty;
                centerDist[k].node = k;
...
inline float HardMultipleClustersAssign(const COMMON::Dataset<T>& data,
    ...
    auto func1 = [&](int tid)
    {
        ...
        float* iclusterDist = args.clusterDist + tid * args._K;
        std::vector<SPTAG::NodeDistPair> centerDist(args._K, SPTAG::NodeDistPair());
        printf("***main.cpp:::HardMultipleClustersAssign\n");
        for (SizeType i = istart; i < iend; i++) {
            for (int k = 0; k < args._K; k++) {
                float dist = args.fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,data[indices[i]], args.centers + k * args._D, args._D);
                centerDist[k].node = k;
                centerDist[k].distance = dist;
            }
            std::sort(centerDist.begin(), centerDist.end(), [](const SPTAG::NodeDistPair& a, const SPTAG::NodeDistPair& b) {

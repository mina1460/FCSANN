...
#undef DefineBKTParameter
        ...
        void Index<std::uint8_t>::SetQuantizer(std::shared_ptr<SPTAG::COMMON::IQuantizer> quantizer)
        {
            ...
            if (m_pQuantizer)
            {
                // m_fComputeDistance = m_pQuantizer->DistanceCalcSelector<std::uint8_t>(m_iDistCalcMethod); //TODO uncomment
...
#define Search(CheckDeleted, CheckDuplicated) \
        std::shared_lock<std::shared_timed_mutex> lock(*(m_pTrees.m_lock)); \
        m_pTrees.InitSearchTrees(m_pSamples, m_fComputeDistance, p_query, p_space, m_filterdataSet_); \
        m_pTrees.SearchTrees(m_pSamples, m_fComputeDistance, p_query, p_space, m_iNumberOfInitialDynamicPivots, m_filterdataSet_); \
        const DimensionType checkPos = m_pGraph.m_iNeighborhoodSize - 1; \
        while (!p_space.m_NGQueue.empty()) { \
            NodeDistPair gnode = p_space.m_NGQueue.pop(); \
            SizeType tmpNode = gnode.node; \
            ...
            for (DimensionType i = 0; i <= checkPos; i++) { \
                ...
                if (nn_index < 0) break; \
                if (p_space.CheckAndSet(nn_index)) continue; \
                float distance2leaf = m_fComputeDistance((const T*)p_query.GetFilterData(), (const T*)(this->GetFilter(nn_index)), (this->GetFilterDim()), (this->GetFilterNum()), p_query.GetQuantizedTarget(), (m_pSamples)[nn_index], GetFeatureDim()); \
                p_space.m_iNumberOfCheckedLeaves++; \
            ...
            if (p_space.m_NGQueue.Top().distance > p_space.m_SPTQueue.Top().distance) { \
                m_pTrees.SearchTrees(m_pSamples, m_fComputeDistance, p_query, p_space, m_iNumberOfOtherDynamicPivots + p_space.m_iNumberOfCheckedLeaves, m_filterdataSet_); \
            } \
        } \
        p_query.SortResult(); \
    ...
    {
            // printf("We are inside BKT index");
        ...
        {
            // printf("$$$BKTIindex.cpp::SearchIndex\n");
        ...
        {
            // printf("$$$BKTIindex.cpp::RefineSearchIndex\n");
            auto workSpace = m_workSpacePool->Rent();
        ...
        {
            // printf("$$$BKTIindex.cpp::SearchTree\n");
            auto workSpace = m_workSpacePool->Rent();
            workSpace->Reset(m_pGraph.m_iMaxCheckForRefineGraph, p_query.GetResultNum());

            COMMON::QueryResultSet<T>* p_results = (COMMON::QueryResultSet<T>*)&p_query;
            m_pTrees.InitSearchTrees(m_pSamples, m_fComputeDistance, *p_results, *workSpace, m_filterdataSet_);
            m_pTrees.SearchTrees(m_pSamples, m_fComputeDistance, *p_results, *workSpace, m_iNumberOfInitialDynamicPivots, m_filterdataSet_);
...
#pragma endregion
        ...
        ErrorCode Index<T>::BuildIndex(const void* p_data, SizeType p_vectorNum, DimensionType p_dimension, bool p_normalized, bool p_shareOwnership)
        {
            ...
            m_pSamples.setFilterdataSet(m_filterdataSet_);
...
#pragma omp parallel for
            ...
            m_pTrees.BuildTrees<T>(m_pSamples, m_iDistCalcMethod, m_iNumberOfThreads, m_filterdataSet_);
...
#undef DefineBKTParameter
            ...
            (*newtree).BuildTrees<T>(ptr->m_pSamples, ptr->m_iDistCalcMethod, omp_get_num_threads(), m_filterdataSet_);
        ...
        ErrorCode Index<T>::RefineIndex(const std::vector<std::shared_ptr<Helper::DiskIO>>& p_indexStreams, IAbortOperation* p_abort)
        {
            ...
            newTrees.BuildTrees<T>(m_pSamples, m_iDistCalcMethod, omp_get_num_threads(), m_filterdataSet_, &indices, &reverseIndices);

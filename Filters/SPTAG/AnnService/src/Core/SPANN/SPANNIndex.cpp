...
#pragma region K-NN search
        ...
        template<typename T>
        ErrorCode Index<T>::SearchIndexWFilters(QueryResult &p_query, bool p_searchDeleted) const
        {
            // printf("entered the correct search function\n");
            if (!m_bReady) return ErrorCode::EmptyIndex;
            
            if (m_filterdataSet == nullptr) {
                // std::shared_ptr<SPTAG::FilterdataSet> filterdataSet = m_filterdataSet_;
                // m_filterdataSet = std::move(filterdataSet);
                // m_filterdataSet = m_filterdataSet_;

                // SizeType p_filtersNum = m_filterdataSet->GetFilterNum();
                // DimensionType p_filtersDimension = m_filterdataSet->GetFilterDim();
                // SizeType p_vectorNum = m_filterdataSet->Count();
                // SizeType p_filtersSize = sizeof(T) * p_filtersNum * p_filtersDimension;
                // m_filterdataSet.reset(new MemFilterdataSet(m_filterdataSet_->GetAllFilterdata(), p_filtersNum, p_filtersDimension, p_vectorNum, p_filtersSize));

                // ByteArray arr1 = ByteArray::Alloc(sizeof(T) * p_filtersNum * p_filtersDimension*p_vectorNum);
                // memcpy(arr1.Data(), p_filters, sizeof(T) * p_filtersNum * p_filtersDimension*p_vectorNum);
                // m_filterdataSet.reset(new MemFilterdataSet( arr1, p_filtersNum, p_filtersDimension, p_vectorNum, sizeof(T)*p_filtersDimension*p_filtersNum));

                printf("m_filterdataSet is null\n");
                if (m_filterdataSet_ == nullptr) {
                    printf("m_filterdataSet_ is null\n");
                }
                m_index->SetFilterForDataset(m_filterdataSet_);
                m_index->SetFilter(m_filterdataSet_);
            }
            ...
            p_queryResults->InitFilters(p_query.GetFilterData(), p_query.GetFilterNum(), p_query.GetFilterDim());
            ...
            {
                SizeType result = p_query.GetResult(i)->VID;
                // printf("%f\n",m_filterdataSet->GetFilterdata(result)[0]);
                p_query.SetFilterData(i, (result < 0) ? ByteArray::c_empty : m_filterdataSet == nullptr ? m_filterdataSet_->GetFilterdata(result) : m_filterdataSet->GetFilterdata(result));
            }
            // printf("%f\n",(float*)p_query.GetFilterData(0).Data());
            // printf("%f\n",p_query.GetFilterData(1)[0]);
            // printf("isnide SearchIndexWFilters\n");
            return ErrorCode::Success;
        }
...
#pragma endregion
        ...
        bool Index<T>::SelectHeadInternal(std::shared_ptr<Helper::VectorSetReader>& p_reader) {
            std::shared_ptr<VectorSet> vectorset = p_reader->GetVectorSet();        // from MemoryReader.h
            ...
            data.setFilterdataSet(m_filterdataSet);
            ...
            if (data.R() == 1) {
                ...
                bkt->BuildTrees<InternalDataType>(data, m_options.m_distCalcMethod, m_options.m_iSelectHeadNumberOfThreads, m_filterdataSet, nullptr, nullptr, true );
        ...
        ErrorCode Index<T>::BuildIndexInternal(std::shared_ptr<Helper::VectorSetReader>& p_reader) {
            ...
            if (m_options.m_buildHead) {
                ...
                m_index->SetFilter(m_filterdataSet);
                m_index->SetQuantizer(m_pQuantizer);
            ...
            if (m_options.m_enableSSD) {
                ...
                m_index->SetParameter("HashTableExponent", std::to_string(m_options.m_hashExp));
                m_index->UpdateIndex();
                m_index->SetFilterForDataset(m_filterdataSet);
                m_index->SetFilter(m_filterdataSet);
                if (m_pQuantizer)
                {
                    m_extraSearcher.reset(new ExtraFullGraphSearcher<std::uint8_t>());
        ...
        // template <typename T>
        // ErrorCode Index<T>::BuildIndexWFilters(bool p_normalized) 
        // {
        //     SPTAG::VectorValueType valueType = m_pQuantizer ? SPTAG::VectorValueType::UInt8 : m_options.m_valueType;
        //     SizeType dim = m_pQuantizer ? m_pQuantizer->GetNumSubvectors() : m_options.m_dim;
        //     std::shared_ptr<Helper::ReaderOptions> vectorOptions(new Helper::ReaderOptions(valueType, dim, m_options.m_vectorType, m_options.m_vectorDelimiter, m_options.m_iSSDNumberOfThreads, p_normalized));
        ...
        //     if (m_options.m_filterVectorPath.empty())
        //     {
        //         LOG(Helper::LogLevel::LL_Info, "Filter vector file is empty. Skipping loading.\n");
        //     }
        //     else {
        //         if (ErrorCode::Success != vectorReader->LoadFilterVectorFile(m_options.m_filterVectorPath))
        //         {
        //             LOG(Helper::LogLevel::LL_Error, "Failed to read filter vector file.\n");
        //             return ErrorCode::Fail;
        //         }
        //         m_options.m_filterVectorSize = vectorReader->GetFilterVectorSet()->FiltersCount();
        //     }
        ...
        template <typename T>
        ErrorCode Index<T>::BuildIndexWFilters(const void* p_data, SizeType p_vectorNum, DimensionType p_dimension, const void* p_filters, SizeType p_filtersNum, DimensionType p_filtersDimension, bool p_normalized, bool p_shareOwnership)
        {
            LOG(Helper::LogLevel::LL_Info, "Entered the BUILD index with filters function \n");
            // if (p_data == nullptr || p_vectorNum == 0 || p_dimension == 0) return ErrorCode::EmptyData;

            // std::shared_ptr<VectorSet> vectorSet;
            // if (p_shareOwnership) {
            //     vectorSet.reset(new BasicVectorSet(ByteArray((std::uint8_t*)p_data, sizeof(T) * p_vectorNum * p_dimension, false),
            //         GetEnumValueType<T>(), p_dimension, p_vectorNum, ByteArray((std::uint8_t*)p_filters, sizeof(T) * p_filtersNum * p_filtersDimension * p_vectorNum, false), p_filtersDimension, p_filtersNum));
            // }
            // else {
            //     ByteArray arr = ByteArray::Alloc(sizeof(T) * p_vectorNum * p_dimension);
            //     memcpy(arr.Data(), p_data, sizeof(T) * p_vectorNum * p_dimension);

            //     ByteArray filtersArr = ByteArray::Alloc(sizeof(T) * p_filtersNum * p_filtersDimension * p_vectorNum);
            //     memcpy(filtersArr.Data(), p_filters, sizeof(T) * p_filtersNum * p_filtersDimension * p_vectorNum);
            //     vectorSet.reset(new BasicVectorSet(arr, GetEnumValueType<T>(), p_dimension, p_vectorNum, filtersArr, p_filtersDimension, p_filtersNum));
            // }


            // if (m_options.m_distCalcMethod == DistCalcMethod::Cosine && !p_normalized) {
            //     vectorSet->Normalize(m_options.m_iSSDNumberOfThreads);
            //     vectorSet->NormalizeFilters(m_options.m_iSSDNumberOfThreads);
            // }
            // SPTAG::VectorValueType valueType = m_pQuantizer ? SPTAG::VectorValueType::UInt8 : m_options.m_valueType;
            // std::shared_ptr<Helper::VectorSetReader> vectorReader(new Helper::MemoryVectorReader(std::make_shared<Helper::ReaderOptions>(valueType, p_dimension, VectorFileType::DEFAULT, m_options.m_vectorDelimiter, m_options.m_iSSDNumberOfThreads, true),
            //     vectorSet));
            ...
            // m_options.m_vectorSize = p_vectorNum;
            // return BuildIndexInternal(vectorReader);
            // printf("%f", ByteArray( (std::uint8_t*)p_data, sizeof(T) * p_vectorNum * p_dimension, false));
            // printf("%f\n",ByteArray((std::uint8_t*) p_filters, p_filtersNum*p_filtersDimension*p_vectorNum*sizeof(T), false )[0]);
            ByteArray arr1 = ByteArray::Alloc(sizeof(T) * p_filtersNum * p_filtersDimension*p_vectorNum);
            memcpy(arr1.Data(), p_filters, sizeof(T) * p_filtersNum * p_filtersDimension*p_vectorNum);
            m_filterdataSet.reset(new MemFilterdataSet( arr1, p_filtersNum, p_filtersDimension, p_vectorNum, sizeof(T)*p_filtersDimension*p_filtersNum));

...
VectorIndex::SaveIndex(const std::string& p_folderPath)
{
    ...
    if (m_filterdataSet_ != nullptr){
        std::string parentFolder = p_folderPath.substr(0, p_folderPath.find_last_of("/\\"));
        m_filterdataSet_->SaveToFile(parentFolder + "/filterdata.bin");
    }
...
VectorIndex::LoadIndex(const std::string& p_loaderFilePath, std::shared_ptr<VectorIndex>& p_vectorIndex)
{
    ...
    // check the a path exists
    // printf("%s\n", (folderPath + "HeadIndex/filterdata.bin").c_str());
    if (fileexists((folderPath + "HeadIndex/filterdata.bin").c_str()) || fileexists((folderPath + "filterdata.bin").c_str()))
    {
        LOG(Helper::LogLevel::LL_Info, "Loading filter data.\n");
        const std::string filterdataPath = fileexists((folderPath + "HeadIndex/filterdata.bin").c_str()) ? (folderPath + "HeadIndex/") : folderPath;
        std::shared_ptr<SPTAG::FilterdataSet> filterdataset (new SPTAG::MemFilterdataSet());
        p_vectorIndex->m_filterdataSet_ = filterdataset;
        if (p_vectorIndex->m_filterdataSet_->LoadFromFile(filterdataPath + "filterdata.bin") != ErrorCode::Success)
        {
            LOG(Helper::LogLevel::LL_Error, "Error: Failed to load filter.\n");
            return ErrorCode::Fail;
        }
    }
...
const void VectorIndex::SetFilterForDataset(const std::shared_ptr<SPTAG::FilterdataSet> filterData) {
    printf("SetFilterForDataset Main\n");
    return;
}

const void* VectorIndex::GetFilter(const SizeType idx) const {
    auto filter = m_filterdataSet_->GetFilterdata(idx);
    return filter.Data();
}

const SizeType VectorIndex::GetFilterNum () const {
    return m_filterdataSet_->GetFilterNum();
}

const DimensionType VectorIndex::GetFilterDim () const {
    return m_filterdataSet_->GetFilterDim();
}

  const void VectorIndex::SetFilter(const std::shared_ptr<SPTAG::FilterdataSet> filterData){
    m_filterdataSet_=filterData ;
  }
...
#undef DefineVectorValueType
                    ...
                    resultSet.Reset();
                    resultSet.SetFilter(GetFilter(fullID));
                    ...
                    for (int i = 0; i < candidateNum && currReplicaCount < replicaCount; ++i)
                    {
                        ...
                        {
                            float nnDist = ComputeDistance(GetFilter(queryResults[i].VID), GetFilter(selections[selectionOffset+j].node),GetSample(queryResults[i].VID), GetSample(selections[selectionOffset+j].node));

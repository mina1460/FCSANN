...
#undef DefineVectorValueType
        ...
        template <>
        std::function<float(const std::uint8_t* , const std::uint8_t* , DimensionType ,SizeType , const std::uint8_t*, const std::uint8_t*, DimensionType)> IQuantizer::DistanceCalcSelector<std::uint8_t>(SPTAG::DistCalcMethod p_method) const
        {
            if (p_method == SPTAG::DistCalcMethod::L2)
            {
                return ([this](const std::uint8_t* pX_f, const std::uint8_t* pY_f, DimensionType dimension_f,SizeType filtersNum, const std::uint8_t* pX, const std::uint8_t* pY, DimensionType dimension) {return L2Distance(pX, pY); });
            }
            else
            {
                return ([this](const std::uint8_t* pX_f, const std::uint8_t* pY_f, DimensionType dimension_f,SizeType filtersNum, const std::uint8_t* pX, const std::uint8_t* pY, DimensionType dimension) {return CosineDistance(pX, pY); });
            }
        }

        template <typename T>
        std::function<float(const T*, const T* , DimensionType ,SizeType , const T*, const T*, DimensionType)> IQuantizer::DistanceCalcSelector(SPTAG::DistCalcMethod p_method) const
        {
            return SPTAG::COMMON::DistanceCalcSelector<T>(p_method);
        }

#define DefineVectorValueType(Name, Type) \
template std::function<float (const Type* , const Type* , DimensionType ,SizeType , const Type* , const Type*, DimensionType)> IQuantizer::DistanceCalcSelector<Type>(SPTAG::DistCalcMethod p_method) const;
#include "inc/Core/DefinitionList.h"
#undef DefineVectorValueType
    }
}

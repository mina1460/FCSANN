...
#pragma omp parallel for
            for (int i = 0; i < querySet->Count(); ++i)
            {
                ...
                {
                    float dist = fComputeDistance((T*)NULL,(T*) NULL, (SPTAG::DimensionType)1,(SPTAG::SizeType)1,query.GetQuantizedTarget(), reinterpret_cast<T*>(vectorSet->GetVector(j)), vectorSet->Dimension());

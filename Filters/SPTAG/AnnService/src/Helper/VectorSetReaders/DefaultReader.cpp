...
std::shared_ptr<FilterdataSet>
DefaultVectorReader::GetFilterdataSet() const
{
    // if (fileexists(m_metadataIndexOutput.c_str()) && fileexists(m_metadataConentOutput.c_str()))
    //     return std::shared_ptr<FilterdataSet>(new FileMetadataSet(m_metadataConentOutput, m_metadataIndexOutput));
    return nullptr;
}

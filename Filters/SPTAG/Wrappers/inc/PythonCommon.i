...
%{
    {
        ...
        auto dstFilter = PyList_New(resNum);
        int i = 0;
        for (const auto& res : *($1))
        {
            ...
            PyList_SetItem(dstFilter, i, PyInt_FromLong(res.VID));

            i++;
        }
        // for (i=0; i<resNum; i++)
        // {
        //     const auto& filter = $1->GetFilterData(i);
        //         PyList_SetItem(dstFilter, i, PyFloat_Unpack4(filter.Data()+i*4, 0));
        // }
        if ($1->WithMeta()) 
        {
            for (i = 0; i < resNum; ++i)
        ...
        PyTuple_SetItem($result, 3, dstFilter);
    }
%}

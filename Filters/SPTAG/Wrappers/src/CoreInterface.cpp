...
bool 
AnnIndex::BuildWithFilters(ByteArray p_data, ByteArray p_filters, SizeType p_vectorNum, SizeType p_filterNum, DimensionType p_filtersDimension)
{
    // printf("%f\n", p_data[0]);
    // printf("%f\n", p_filters[0]);

    // printf("%d, %d\n", p_filterNum, p_filtersDimension);
    // printf("%d\n", p_filters.Length());
    // printf("%d, %d\n", p_data.Length(), p_vectorNum * m_inputVectorSize);

    // unsigned char * f = p_filters.Data();
    // for (int i=0; i<p_filterNum*p_filtersDimension; i++){
    //     printf("%d ", *f);
    //     f += sizeof(*f);
    // }
    // printf("****\n");
    ...
    if (nullptr == m_index || p_vectorNum == 0 || m_dimension == 0 || p_data.Length() != p_vectorNum * m_inputVectorSize)
    {
        // LOG(Helper::LogLevel::LL_Info, "Begin Adjust Parameters...\n");
        printf("Failed inside the buildfilters function \n");
        return false;
    }
    ...
    //     vectorType,
    //     static_cast<SPTAG::DimensionType>(vectorSize),
    //     static_cast<SPTAG::SizeType>(p_vectorNum)));

    // std::shared_ptr<SPTAG::FilterdataSet> filters(new SPTAG::MemFilterdataSet(p_filters,
    //     static_cast<SPTAG::SizeType>(p_filterNum),
    //     static_cast<SPTAG::DimensionType>(p_filtersDimension),
    //     static_cast<SPTAG::SizeType>(p_vectorNum)));

    return (SPTAG::ErrorCode::Success == m_index->BuildIndexWFilters(p_data.Data(), p_vectorNum,  m_dimension, p_filters.Data(), p_filterNum, p_filtersDimension));
}
...
std::shared_ptr<QueryResult>
AnnIndex::SearchWithFilters(ByteArray p_data, ByteArray p_filters, SizeType p_filterNum, DimensionType p_filtersDimension, int p_resultNum){
    ...
    // for (int i=0; i<p_filterNum*p_filtersDimension; i++){
    //     printf("%d ", *f);
    //     f += sizeof(*f) * sizeof(int);
    // }
    // printf("****\n");
    std::shared_ptr<QueryResult> results = std::make_shared<QueryResult>(p_data.Data(), p_resultNum, false);
    results->InitFilters(p_filters.Data(), p_filterNum, p_filtersDimension);

    if (nullptr != m_index)
    {
        // printf("Inside the searchwithfilters function \n");
        // m_index->SearchIndexWFilters(*results, p_filters.Data(), p_filterNum, p_filtersDimension);
        if (m_index->SearchIndexWFilters(*results)==SPTAG::ErrorCode::Success){

...
bool AnnIndex::BuildWithMetaData(ByteArray p_data, ByteArray p_meta, SizeType p_num, bool p_withMetaIndex, bool p_normalized)
{
    if (nullptr == m_index)
    {
        m_index = SPTAG::VectorIndex::CreateInstance(m_algoType, m_inputValueType);
...
AnnIndex::FakasuloSearchProdCons(ByteArray p_data, int p_vectorNum, int p_resultNum, bool p_withMetaData, int C_num_threads, int P_num_threads)
{

    int num_threads = C_num_threads;
    ...
    m_index->ThreadedSelectHeads(queries, C_num_threads, P_num_threads);
...
bool AnnIndex::AddWithMetaData(ByteArray p_data, ByteArray p_meta, SizeType p_num, bool p_withMetaIndex, bool p_normalized)
{
    if (nullptr == m_index)
    {
        m_index = SPTAG::VectorIndex::CreateInstance(m_algoType, m_inputValueType);

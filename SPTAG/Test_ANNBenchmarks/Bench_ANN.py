...
class ANN:
    ...
    def query(self, algorithm) -> np.ndarray:
        ...
        if algorithm == 'spann':
            ...
            result = self.index.FakasuloSearch(self.data.test, self.data.test.shape[0], self.n_neighbors, False, 4)
            result = np.array(result[0]).reshape((self.data.test.shape[0], self.n_neighbors))
    ...
    def k_recall_at_k(self, labels, ground_truth) -> float:
        ...
        ground_truth = np.concatenate((ground_truth, ground_truth), axis=0)
        ground_truth = np.concatenate((ground_truth, ground_truth), axis=0)
        ground_truth = np.concatenate((ground_truth, ground_truth), axis=0)
        ground_truth = np.concatenate((ground_truth, ground_truth), axis=0)
